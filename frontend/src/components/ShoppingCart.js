...
function ShoppingCart({key,itemImg,itemName,orgPrice,discountedPrice,discount}) {
    ...
    return(
        <div className="min-w-44 max-w-48 relative p-1 rounded-md border border-gray-200 shadow-sm snap-center flex flex-col gap-1.5 hover:shadow-md hover:shadow-gray-300 x-sm:hover:shadow-none x-sm:shadow-white cursor-pointer" data={cartData} id='cart'>
            <div className={`absolute z-10 rounded-sm font-bold text-white shadow-md px-1.5 p-1 capitalize text-sm ${discount==='new'?"bg-green-600":'bg-red-500'}`} 
